...
- **Mobile / responsive**: done. The site is fully responsive down to ~360px. The navigation collapses into a hamburger menu below 900px, all multi-column sections stack to a single column on phones, headings scale down, and the page never scrolls sideways. The responsive rules live at the bottom of `css/styles.css` (media queries) plus the hamburger toggle in `js/main.js`. Verified with no horizontal overflow on every page at 375px.
